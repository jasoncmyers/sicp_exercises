...
; Requires <blah> from previous exercises
; Original expmod for reference:
...
(define (expmod-signal base exp m)
    (define (check-nontrivial a n value)
      (cond
        ((not (= value 1))
         value)
        ((= a 1)
    ...
    (cond
      ((= exp 0) 1)
      ((even? exp)
       (remainder (check-nontrivial base exp (square (expmod base (/ exp 2) m))) m))
      (else
